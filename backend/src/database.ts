...
export const initDatabase = async () => {
  try {
    ...
    await db.exec(`
      ...
      VALUES 
        ('test-user', 'Daily Transactions', 'SELECT COUNT(*) as tx_count, DATE(timestamp) as date FROM starknet_transactions GROUP BY date ORDER BY date DESC LIMIT 7'),
        ('test-user', 'Top Contracts', 'SELECT contract_address, COUNT(*) as calls FROM starknet_transactions GROUP BY contract_address ORDER BY calls DESC LIMIT 10');
    `);
